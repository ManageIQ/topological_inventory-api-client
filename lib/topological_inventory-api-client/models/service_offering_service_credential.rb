...
module TopologicalInventoryApiClient
  class ServiceOfferingServiceCredential
    ...
    # ID of the resource
    attr_accessor :service_credential_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TopologicalInventoryApiClient::ServiceOfferingServiceCredential` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TopologicalInventoryApiClient::ServiceOfferingServiceCredential`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] service_credential_id Value to be assigned
    def service_credential_id=(service_credential_id)
      pattern = Regexp.new(/^\d+$/)
      if !service_credential_id.nil? && service_credential_id !~ pattern
        fail ArgumentError, "invalid value for \"service_credential_id\", must conform to the pattern #{pattern}."
      end
    ...
    def hash
      [created_at, id, last_seen_at, service_credential_id, service_offering_id, updated_at].hash
    end
