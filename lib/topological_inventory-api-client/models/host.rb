  ...
  class Host
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TopologicalInventoryApiClient::Host` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TopologicalInventoryApiClient::Host`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@cluster_id.nil? && @cluster_id !~ pattern
        invalid_properties.push("invalid value for \"cluster_id\", must conform to the pattern #{pattern}.")
      end
    ...
    def cluster_id=(cluster_id)
      pattern = Regexp.new(/^\d+$/)
      if !cluster_id.nil? && cluster_id !~ pattern
        fail ArgumentError, "invalid value for \"cluster_id\", must conform to the pattern #{pattern}."
      end
