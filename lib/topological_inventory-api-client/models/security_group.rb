  ...
  class SecurityGroup
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TopologicalInventoryApiClient::SecurityGroup` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TopologicalInventoryApiClient::SecurityGroup`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^\d+$/)
      if !@network_id.nil? && @network_id !~ pattern
        invalid_properties.push("invalid value for \"network_id\", must conform to the pattern #{pattern}.")
      end
    ...
    def network_id=(network_id)
      pattern = Regexp.new(/^\d+$/)
      if !network_id.nil? && network_id !~ pattern
        fail ArgumentError, "invalid value for \"network_id\", must conform to the pattern #{pattern}."
      end
