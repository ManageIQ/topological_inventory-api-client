  ...
  class Container
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TopologicalInventoryApiClient::Container` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TopologicalInventoryApiClient::Container`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'container_group_id')
    ...
    def list_invalid_properties
      ...
      if !@container_group_id.nil? && @container_group_id !~ pattern
        invalid_properties.push("invalid value for \"container_group_id\", must conform to the pattern #{pattern}.")
      end

      pattern = Regexp.new(/^\d+$/)
      if !@container_image_id.nil? && @container_image_id !~ pattern
        invalid_properties.push("invalid value for \"container_image_id\", must conform to the pattern #{pattern}.")
      end
    ...
    def container_group_id=(container_group_id)
      pattern = Regexp.new(/^\d+$/)
      if !container_group_id.nil? && container_group_id !~ pattern
        fail ArgumentError, "invalid value for \"container_group_id\", must conform to the pattern #{pattern}."
      end
    ...
    def container_image_id=(container_image_id)
      pattern = Regexp.new(/^\d+$/)
      if !container_image_id.nil? && container_image_id !~ pattern
        fail ArgumentError, "invalid value for \"container_image_id\", must conform to the pattern #{pattern}."
      end
