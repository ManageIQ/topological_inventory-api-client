...
module TopologicalInventoryApiClient
  class DefaultApi
    ...
    # @option opts [Object] :filter Filter for querying collections.
    # @option opts [OneOfstringarray] :sort_by The list of attribute and order to sort the result set by.
    # @return [ServiceOfferingServiceCredentialsCollection]
    def list_service_offering_service_offering_service_credentials(id, opts = {})
      data, _status_code, _headers = list_service_offering_service_offering_service_credentials_with_http_info(id, opts)
      data
    end
    ...
    # @option opts [Object] :filter Filter for querying collections.
    # @option opts [OneOfstringarray] :sort_by The list of attribute and order to sort the result set by.
    # @return [Array<(ServiceOfferingServiceCredentialsCollection, Integer, Hash)>] ServiceOfferingServiceCredentialsCollection data, response status code and response headers
    def list_service_offering_service_offering_service_credentials_with_http_info(id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: DefaultApi.list_service_offering_service_offering_service_credentials ...'
      end
      # verify the required parameter 'id' is set
      if @api_client.config.client_side_validation && id.nil?
        fail ArgumentError, "Missing the required parameter 'id' when calling DefaultApi.list_service_offering_service_offering_service_credentials"
      end
      pattern = Regexp.new(/^\d+$/)
      if @api_client.config.client_side_validation && id !~ pattern
        fail ArgumentError, "invalid value for 'id' when calling DefaultApi.list_service_offering_service_offering_service_credentials, must conform to the pattern #{pattern}."
      end

      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] > 1000
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling DefaultApi.list_service_offering_service_offering_service_credentials, must be smaller than or equal to 1000.'
      end

      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] < 1
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling DefaultApi.list_service_offering_service_offering_service_credentials, must be greater than or equal to 1.'
      end

      if @api_client.config.client_side_validation && !opts[:'offset'].nil? && opts[:'offset'] < 0
        fail ArgumentError, 'invalid value for "opts[:"offset"]" when calling DefaultApi.list_service_offering_service_offering_service_credentials, must be greater than or equal to 0.'
      end

      # resource path
      local_var_path = '/service_offerings/{id}/service_offering_service_credentials'.sub('{' + 'id' + '}', CGI.escape(id.to_s))
      ...
      # return_type
      return_type = opts[:return_type] || 'ServiceOfferingServiceCredentialsCollection' 
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: DefaultApi#list_service_offering_service_offering_service_credentials\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
